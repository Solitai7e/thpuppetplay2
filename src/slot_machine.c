...
static void (*const sReelTimeActions[])(struct Task *task) =
{
    ...
    ReelTime_WaitStartCCirno,
    ReelTime_CCirnoSpeedUp1,
    ReelTime_CCirnoSpeedUp2,
    ReelTime_WaitReel,
    ReelTime_CheckExplode,
    ReelTime_LandOnOutcome,
    ReelTime_CCirnoReact,
...
static bool8 SlotAction_FreeDataStructures(struct Task *task)
{
    if (!gPaletteFade.active)
    {
        ...
        if (sImageTable_ReelTimeCCirno != NULL)
            FREE_AND_SET_NULL(sImageTable_ReelTimeCCirno);
...
static const u8 sSymToMatch[] = {
    ...
    [GFXTAG_CSHINGYOKU] = MATCHED_CSHINGYOKU,
    [GFXTAG_AHINA]   = MATCHED_AHINA,
...
static const u16 sSlotMatchFlags[] = {
    ...
    [MATCHED_AHINA]     = 1 << MATCHED_AHINA,
    [MATCHED_CSHINGYOKU]   = 1 << MATCHED_CSHINGYOKU,
